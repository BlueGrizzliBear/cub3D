...
t_cursor	*ft_new_cursor(t_win *win)
{
	...
	if ((cursor = malloc(sizeof(t_cursor))) == NULL)
		ft_error(-31, win);
	cursor->img = ft_new_image(win);
	cursor->img->b_color = ft_new_clr(255, 255, 255, 255);
	cursor->color = ft_new_clr(CURSOR_ALPHA,
			CURSOR_RED, CURSOR_GREEN, CURSOR_BLUE);
	return (cursor);
}
