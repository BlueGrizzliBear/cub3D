...
t_hud		*ft_new_hud(t_win *win)
{
	...
	if ((hud = malloc(sizeof(t_hud))) == NULL)
		ft_error(-31, win);
	hud->transp = 0;
	hud->red_path = "./textures/hud_red.xpm";
	hud->red_img = ft_new_image(win);
	hud->red_screen = ft_new_image(win);
	hud->white_path = "./textures/hud_white.xpm";
	hud->white_img = ft_new_image(win);
	hud->white_screen = ft_new_image(win);
	hud->blood_path = "./textures/blood_hud.xpm";
	hud->blood_img = ft_new_image(win);
	hud->blood_screen = ft_new_image(win);
	hud->dead_path = "./textures/dead_hud.xpm";
	hud->dead_img = ft_new_image(win);
	hud->dead_screen = ft_new_image(win);
	return (hud);
}
...
t_win		*ft_new_window(void)
{
	...
	win->cursor = ft_new_cursor(win);
	win->hud = ft_new_hud(win);
